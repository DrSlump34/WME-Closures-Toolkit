        ...
        for(let closureIdx=0;closureIdx<e.closures.length;closureIdx++){
            ...
            await new Promise(res=>{
                addClosure({segments:activeSegs,reason:e.config.reason,direction:dir,startDate:cl.start,endDate:cl.end,permanent:e.config.ignoretraffic,eventId:e.config.mteId||null},
                    ()=>{done++;upd(done+failed);const ls=cl.start instanceof Date?formatDateDisplay(cl.start):cl.start;logApply(t('applyOk',e.config.reason,ls),'#43a047');res();},
                    (errs)=>{failed++;upd(done+failed);const ls=cl.start instanceof Date?formatDateDisplay(cl.start):cl.start;logApply(t('applyErr',e.config.reason,ls,errs[0]||'error'),'#e53935');res();});
            });
        }
    }
...
const doInjectFab=()=>{
    ...
    // Appliquer position sauvegardée ou défaut
    const pos=(_fabX>=0&&_fabY>=0)?{x:_fabX,y:_fabY}:_fabDefaultPos();
